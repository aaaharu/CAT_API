...
struct PhotoItem {
    ...
    init(_ result:Result) {
        self.urlString = result.urls?.regular ?? ""
        self.name = result.id ?? ""
    }
}
...
struct CoverPhotoTopicSubmissions: Codable {
    let animals, texturesPatterns, colorOfWater: Animals?
...
struct ResultTopicSubmissions: Codable {
    let wallpapers, animals: Animals?
    let nature, film, fashionBeauty: FashionBeauty?

    enum CodingKeys: String, CodingKey {
        case wallpapers, animals, nature, film
